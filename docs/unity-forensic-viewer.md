...
The committed native-generated fixture is shared by loader, timeline, playback,
workspace, viewer-session, controller, and Editor-layer tests.
...
The v0.7.0 viewer includes:

- a Unity 6 project with separate runtime, Editor, and EditMode test assemblies;
- a schema-versioned investigation-bundle contract;
- deterministic native C++ JSON export from verified SQLite investigations;
- byte-for-byte native export compatibility coverage;
- file and in-memory JSON loading;
- stable validation findings;
- strict schema, identity, integer, digest, ordinal, session, chain, and replay-count validation;
- deterministic replay timeline construction;
- nearest-tick seeking;
- play, pause, step, reset, seek, and playback-speed control;
- session and evidence inspection models;
- accepted and rejected outcome filters;
- exact client and session filtering;
- visible-record to source-timeline selection mapping;
- evidence-link and trusted-head inspection;
- replay verification status;
- transactional bundle replacement that preserves the active investigation after a failed load;
- a Unity Editor forensic replay window;
- command, session, archive, replay, and digest inspection panels;
- a WSL-compatible Unity test runner that stages the project on the Windows temporary filesystem;
- Unity EditMode coverage for loading, validation, timeline behavior, playback, filtering, lifecycle, and Editor-controller behavior.

## v0.7.0 delivery boundary

The release provides a read-only Unity Editor investigation client. It does not execute authoritative commands inside Unity, alter evidence, repair invalid bundles, or reinterpret rejected native decisions.

The native C++ layer remains authoritative for archive verification, persistence, replay, and bundle export. Unity consumes the exported investigation contract and presents deterministic inspection state.
